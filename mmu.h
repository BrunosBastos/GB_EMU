#ifndef _MMU_H_
#define _MMU_H_

#include "types.h"
#include "cartridge.h"


class mmu {

    public:

        void initialize();
        void load_rom(cartridge *c);
        byte read_memory(int addr);
        void write_memory(int addr, byte value);

        byte bios[0x100] = {
             
        };

        byte address[0x10000];

        unsigned short rom[2] = {0x0000, 0x4000};   // ROM banks, there are 2 of size 16kB
        unsigned short vram = 0x8000;               // video RAM $8000-9FFF
        unsigned short eram = 0xA000;               // External (Cartridge) RAM 8kB $A000-BFFF
        unsigned short wram = 0xC000;               // Work RAM 8kB $C000-DFFF
        unsigned short oam = 0xFE00;                // Object Attribute memory, 160 bytes
        unsigned short iop = 0xFF00;                // I/O ports $FF00-FF4C, 76 bytes, there is some extra space but is unusable
        unsigned short ppu = 0xFF40;                // usefull for the graphics
        unsigned short hram = 0xFF80;               // high RAM (zero page) $FF80-FFFE, 127 bytes, quick access RAM            
        unsigned short ief = 0xFFFF;                // interrupt enable flag $FFFF, I/O register that controls the interrupts

        /*
        byte rom[2][0x4000];        // ROM banks, there are 2 of size 16kB
        byte vram[0x2000];          // video RAM $8000-9FFF
        byte wram[0x2000];          // Work RAM 8kB $C000-DFFF
        byte eram[0x2000];          // External (Cartridge) RAM 8kB $A000-BFFF
        byte oam[0xA0];             // Object Attribute memory, 160 bytes
        byte iop[0x4C];             // I/O ports $FF00-FF4C, 76 bytes, there is some extra space but is unusable
        byte hram[0x7F];            // high RAM (zero page) $FF80-FFFE, 127 bytes, quick access RAM            
        byte interrupt_enable;      // interrupt enable flag $FFFF, I/O register that controls the interrupts
        // byte empty[0x60];
        */
};



/*
    DETAILS:
    
        OAM {
            OAM is sprite RAM. This area is 40 sprites x 4 bytes long.
            To Display a sprite you write 4 corresponding bytes to OAM.
            These 4 bytes are:
                - byte 1: X location
                - byte 2: Y location
                - tile #: (0-255)
                - attributes

            The tile number is taken from Character RAM.
            The sprites positions are offset by 8 and 16 for X and Y, respectively.
            To set a sprite at 0,0 it must be set to 8, 16.
        }

        External RAM {
            Some programs make use of the RAM available in the cartridge.
        }



*/

#endif
